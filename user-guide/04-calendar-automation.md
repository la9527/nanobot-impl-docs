...
현재 locale 설정과 channel 에 따라 문구는 조금 달라질 수 있지만, 보통 아래처럼 부족한 값을 더 달라고 안내한다.
...
보통 thread 나 WebUI 상태에는 아래 같은 approval 제목 또는 요약이 보인다.
...
approval title, summary, pending button 은 channel 과 surface 에 따라 조금 다르게 보일 수 있다.

```text
Calendar update approval required
Approval required before updating '치과'.
```
...
삭제도 수정과 같은 방식으로 approval title 또는 요약이 보인다.

```text
Calendar delete approval required
Approval required before deleting '치과'.
```
...
- approval 제목, summary, input prompt 는 locale 설정과 surface 에 따라 영어 또는 한국어 중심으로 보일 수 있다.
