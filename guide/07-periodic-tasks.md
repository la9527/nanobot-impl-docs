...
또한 heartbeat 실행 자체는 내부 `heartbeat` 세션과 `cli/direct` 채널에서 처리하고, 사용자에게 보여 줄 최종 응답만 실제 target channel 로 전달하도록 정리됐다. 이 구조는 WebUI 나 Telegram 에 heartbeat 내부 prompt 문구가 그대로 노출되는 일을 막기 위한 것이다.
...
- 어느 경우에도 heartbeat 내부 지시문이나 중간 문구는 target channel 에 직접 노출되지 않고, 최종 사용자 응답만 전달된다.
...
- heartbeat 결과가 WebUI 에 보이더라도 내부 실행 로그나 지시문이 아니라 최종 사용자 응답만 노출되는 것이 정상 동작이다.
