...
현재 메모리 보정은 phrase-driven 구조화 입력이 가장 안정적으로 동작한다.
...
이 프로젝트는 끝났어
프로젝트 메모: 지난 캘린더 테스트 작업은 종료

잊어
내용: 지난번 임시 캘린더 테스트 메모
```

WebUI 에서 memory correction quick action 을 누르면 위 형태의 draft 가 자동으로 들어간다.

짧은 일반 자연어도 보조적으로 해석될 수 있지만, 현재 구현 기준으로는 위처럼 field 이름까지 포함한 입력이 가장 확실하다.
