...
- `docs/observable-reasoning-plan/README.md` 를 기준 entry 로 유지했다.
- `docs/observable-reasoning-plan/telegram-v2-and-reasoning-visibility-plan-2026-05-10.md` 에 정의된 `reasoning_visibility = off | status_only | summary | debug_trace` 정책을 구현 기준으로 삼았다.
...
- 이 이슈는 `docs/todo.md` 에 후속 점검 항목으로도 기록했다.
...
- source repo working tree 는 현재 clean 상태다.
- 다만 `/Volumes/ExtData/Nanobot/docs/status-summary/*` 와 `/Volumes/ExtData/Nanobot/.github/instructions/*` 는 source repo 밖 경로이므로, 이번 status-summary 갱신과 i18n 기본 instruction 추가는 같은 source git commit 에 포함되지 않는다. `/Volumes/ExtData/Nanobot/docs` 자체는 별도 git repo 이므로 docs 변경은 별도 commit 으로 추적할 수 있다.
...
- `/Volumes/ExtData/Nanobot/docs/user-guide/10-webui-terminology.md` 를 추가해 `대시보드`, `설정`, `세부 정보`, `대화`, `작업`, `타깃` 같은 현재 WebUI 기준 표현을 정리했다.
