...
- WebUI 가 owner-aware summary, task summary, action result preview, memory correction draft injection 을 표시하도록 보강됐지만, 상단 정보 밀도가 높아 thread 화면이 과밀해지는 문제가 확인돼 별도 UX 재설계 검토 문서를 `docs/archive/webui-ux-redesign/` 아래에 추가했다.
- `docs/planning/todo.md` 와 `docs/archive/assistant-direction-a/**` 는 active backlog 와 archive/reference 경계를 다시 정리했지만, 이 문서군은 현재 git root 밖에 있으므로 이번 push 범위에는 포함되지 않는다.

## 현재 상태 요약

- git 기준 실제 저장소 루트는 `/Volumes/ExtData/Nanobot/source` 이고, 현재 브랜치는 `feature/nanobot-fork-runtime` 이다.
- `docs/status-summary` 와 `docs/planning/todo.md`, `docs/archive/assistant-direction-a/**` 는 `source` 저장소 바깥 운영 문서 영역이다.
...
- `/Volumes/ExtData/Nanobot/docs/planning/todo.md` 에서 우선순위 2 `Telegram WebUI websocket push mirror 검증 및 마무리` 는 이미 동작 확인이 끝난 항목으로 닫았다.
- `docs/archive/assistant-direction-a/00~08`, `workplans/05-1~05-5` 문서 상단에 archive/reference 상태 메모를 넣어 active source of truth 가 section 5 + `docs/planning/execution-backlog/*.md` 임을 명시했다.
...
- 이에 따라 `docs/archive/webui-ux-redesign/README.md`, `docs/archive/webui-ux-redesign/thread-surface-redesign-2026-05-01.md` 를 추가해 대안 비교, dashboard 검토, 추천안, 단계별 작업안을 문서화했다.
...
- `/Volumes/ExtData/Nanobot/docs/operations/status-summary`, `/Volumes/ExtData/Nanobot/docs/planning/todo.md`, `/Volumes/ExtData/Nanobot/docs/archive/assistant-direction-a/**` 는 같은 프로젝트 영역이지만 git commit/push 대상은 아니다.
...
- `docs/planning/todo.md` 와 `assistant-direction-a` 재정리 내용은 source repo push 와 별개라, 다른 환경에서 git pull 만으로는 따라오지 않는다.
...
1. `/model smart-router` 를 Telegram 또는 실제 외부 채널 세션에서 다시 end-to-end 검증하고 `docs/planning/todo.md` 1번을 닫을지 판단
...
4. git root 밖 운영 문서(`docs/planning/todo.md`, `assistant-direction-a/**`)를 어떻게 버전 관리할지 별도 운영 원칙 정리
